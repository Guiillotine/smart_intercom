...
class S3Connect(IS3Connect):
    ...
    def _get_client(self) -> AioBaseClient:
        return aioboto3.Session().client(
            service_name="s3",
            endpoint_url=self._settings.s3.S3_ENDPOINT_URL,
            aws_access_key_id=self._settings.s3.S3_ACCESS_KEY,
            aws_secret_access_key=self._settings.s3.S3_SECRET_KEY,
            region_name=self._settings.s3.S3_REGION_NAME,
            config=Config(
